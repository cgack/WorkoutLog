...
class IOSRunner(object):
	...
	def setup_simulator(self):
		...
		if not path.exists(self.sdk):
			raise IOSError("Couldn't find the iPhone sdk at: %s" % self.sdk) 
	...
	def create_ipa_from_app(self, build, provisioning_profile, certificate_to_sign_with=None, relative_path_to_itunes_artwork=None):
		...
		generate_dynamic_root = path.abspath(path.join(self.path_to_ios_build, '..', '.template', 'generate_dynamic'))
		...
		with lib.cd(temp_dir):
			...
			path_to_embedded_profile = path.abspath(path.join(path_to_payload, app_folder_name, embedded_profile))
			path_to_payload_app = path.abspath(path.join(path_to_payload, app_folder_name))
			path_to_resource_rules = path.abspath(path.join(path_to_payload_app, 'ResourceRules.plist'))
